...
class LotusPlot:
    ...
    def _generate_field_plot_for_label(self, dataSet: DataSet, label: str, ax: Axes, rgb_color, field: str, marker: str, linestyle: str):
        ...
        `field`: str, {'min', 'max', 'avg', 'var', 'mean', 'std'}
    ...
    def _generate_min_plot_for_label(self, dataSet: DataSet, label: str, ax: Axes, rgb_color):
        self._generate_field_plot_for_label(
            dataSet, label, ax, rgb_color, 'min', 'o', 'dotted')

    def _generate_max_plot_for_label(self, dataSet: DataSet, label: str, ax: Axes, rgb_color):
        self._generate_field_plot_for_label(
            dataSet, label, ax, rgb_color, 'max', 'o', 'dotted')

    def _generate_avg_plot_for_label(self, dataSet: DataSet, label: str, ax: Axes, rgb_color):
        self._generate_field_plot_for_label(
            dataSet, label, ax, rgb_color, 'avg', '^', 'solid')

    def _generate_simp_plot_for_label(self, dataSet: DataSet, label: str, ax: Axes, rgb_color):
        self._generate_min_plot_for_label(dataSet, label, ax, rgb_color)
        self._generate_max_plot_for_label(dataSet, label, ax, rgb_color)
        self._generate_avg_plot_for_label(dataSet, label, ax, rgb_color)

    def _generate_min_plot_for_dataset(self, dataSet: DataSet, fig: Figure, ax: Axes, rgb_color=None):
        if rgb_color is None:
            ...
            colors = (self._lightenColor(rgb_color, min_factor + i * step)
                      for i in range(len(dataSet.Labels)))
        for i, label in enumerate(dataSet.Labels):
            self._generate_avg_plot_for_label(dataSet, label, ax, colors[i])

    def _generate_simp_plot_for_dataset(self, dataSet: DataSet, fig: Figure, ax: Axes, rgb_color=None):
        if rgb_color is None:
            colors = self.COLORS
        else:
            min_factor = 0.5
